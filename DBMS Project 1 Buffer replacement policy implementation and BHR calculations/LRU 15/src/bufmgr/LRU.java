...
//replace Policy above with impemented policy name (e.g., Lru, Clock) -- done |Pratik 09/12|
  ...
  private void update(int frameNo)
  {
    ...
    // LRU replacement policy: For LRU we are treating the frames array as Queue
    // and the item in frames array with the index frameNo contains the index of the most recently used
    // so move that item to the end of the frames array and shift all the other items in the array
    // to the left
    ...
    for (int i = frameNo + 1; i < frames.length; i++){
      frames[i - 1] = frames[i];
    }
 ...
 public int pickVictim()
 {
    ...
    if (nframes <= this.frametab.length - 1){
    ...
    // Here we are using the LRU replacement policy
    for (int i: frames){
