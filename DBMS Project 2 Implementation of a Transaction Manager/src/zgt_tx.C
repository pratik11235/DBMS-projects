...
void *readtx(void *arg){
  ...
  //2. Set the lock on the object as shared lock considering the necessary conditions
  // before acquiring the lock
  zgt_tx *tx = get_tx(node->tid);  // get the transaction id first
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  //2. Set the lock on the object as exclusive lock considering the necessary conditions
  // before acquiring the lock
  zgt_tx *tx = get_tx(node->tid);  // get the transaction id first
...
void *aborttx(void *arg)
{
  ...
  //2. call commit abort function for abort using 'A'
...
void *committx(void *arg)
{
  ...
  //2. call commit abort function for commit using 'C'
...
void *do_commit_abort(long t, char status){
  ...
  if (status == 'C'){
    ...
    if (tx != NULL){
    tx->free_locks();
    tx->end_tx(); // end transaction to commit the changes to disk
    //tx->remove_tx();
    if (zgt_nwait(tx->tid) > 0){
      zgt_v(tx->tid);
    }
    }else{
      printf("ERROR in do_commit_abort() function. Tx %ld does not exist.", t);
      fflush(stdout);
    }
    ...
    if (tx != NULL){
    tx->free_locks();
    tx->remove_tx();  // remove transaction to abort the transaction and no to save changes to the disk
    if (zgt_nwait(tx->tid) > 0){
      zgt_v(tx->tid);
    }
    }else{
  ...
  // remove the transaction and all the locks held by the transaction
  if (tx != NULL){
    tx->free_locks();
    tx->remove_tx();
    if (zgt_nwait(tx->tid) > 0){
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  //if successful  return(0); else -1
    ...
    //write your code
  zgt_tx *tx = get_tx(tid1);  // get the transaction from tid
  // lock the transaction manager before checking if the object is present in the lock hashtable
  zgt_p(0);
  zgt_hlink *obj_lock = ZGT_Ht->find(sgno1, obno1);  // find the obj_lock if lock exist in the lock table
  zgt_v(0);  // release the lock on TM

  if (obj_lock == NULL){ // there is no existing lock on the object then 
    // just lock the TM, add lock for current tx and perform readwrite operation
    zgt_p(0);  // lock transaction manager
    ZGT_Ht->add(tx, sgno1, obno1, lockmode1);  // add the lock for current tx
    zgt_v(0);  // release lock on transaction manager

    tx->perform_readWrite(tid1, obno1, lockmode1); // perform the read write operation as lock is acqquired
  }
  else if (obj_lock->tid == tid1){ // the current transaction is already having the lock
    tx->perform_readWrite(tid1, obno1, lockmode1); // perform the read write operation as lock is already acqquired
  }
  else{  // the object is currently held by another transaction
  // so assign a semaphore and make the current transaction wait till the current transaction gets completed
  int wait_tx_no = zgt_nwait(obj_lock->tid);
  /*
  zgt_p(0);
  zgt_hlink *own_stat = ZGT_Ht->findt(tid1, sgno1, obno1);
  zgt_v(0);
  ...
  */
  zgt_tx *lock_tx = get_tx(obj_lock->tid);

  //if(lockmode1 != 'S' || // current transaction needs exclusive lock
  //(lockmode1 == 'S' && obj_lock->lockmode != 'S') || // curr tx needs S lock but transaction holding lock is X
  //(lockmode1 == 'S' && obj_lock->lockmode == 'S' && wait_tx_no > 0)){ // curr tx & holding tx but have S lock but more transactions are waiting for same object
  
  if(lockmode1 != 'S' || // current transaction needs exclusive lock
  (lockmode1 == 'S' && lock_tx->Txtype != 'R') || // curr tx needs S lock but transaction holding lock is X
  (lockmode1 == 'S' && lock_tx->Txtype == 'R' && wait_tx_no > 0)){ // curr tx & holding tx but have S lock but more transactions are waiting for same object
    // if any of the above condditions is true then 
    tx->obno = obno1;
    tx->lockmode = lockmode1;
    tx->status = TR_WAIT;      // set status of the tx to wait

    // set a semaphore to the transaction and make it wait on tx holding the lock
    tx->setTx_semno(obj_lock->tid, obj_lock->tid);  
    zgt_p(obj_lock->tid);

    // assign undefined values to the waiting transaction (curr tx) as mentioned in the proj description
    tx->obno = -1;
    tx->lockmode = ' ';
    //tx->Txtype = ' ';
    //tx->semno = -1;
    ...
    // if the transaction tx is active then perform readwrite
    tx->perform_readWrite(tid1, obno1, lockmode1);

    zgt_v(obj_lock->tid);
  }
  else{ // current transaction requires a S lock and no other transaction is waiting on the object
    // or holding X lock on the object
    tx->perform_readWrite(tid1, obno1, lockmode1);
  }
  //}
  }
  return(0);  // return 0 is successful
}
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode){
  ...
  //write your code
  //printf("In readWrite function. doing nothing.");
  //fflush(stdout);
  //zgt_tx
  int z;
  double res = 0.0;
  ...
  if (lockmode == 'S'){
    ...
    fflush(ZGT_Sh->logfile);
    //printf("S lock optime %d", ZGT_Sh->optime[tid]);
    //fflush(stdout);
    sleep(ZGT_Sh->optime[tid]/10000);  // wait for optime[tid]/10 milliseconds
    //for(int z=0; z<ZGT_Sh->optime[tid]*20; z++)
    ...
    fflush(ZGT_Sh->logfile);
    //printf("X lock optime %d", ZGT_Sh->optime[tid]);
    //fflush(stdout);
    sleep(ZGT_Sh->optime[tid]/10000);  // wait for optime[tid]/10 milliseconds
    //for(int z=0; z<ZGT_Sh->optime[tid]*20; z++)
		//{res = res + (double) random();}
...
void *finish_operation(long tid){
  ...
  pthread_cond_broadcast(&ZGT_Sh->condpool[tid]);// other waiting threads of same tx // unblock all threads currently blocked
