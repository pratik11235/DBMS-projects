...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  zgt_p(0);
  zgt_hlink *obj_lock = ZGT_Ht->find(sgno, obno);  // find the obj_lock if lock exist in the lock table
  ...
  else if (obj_lock->tid == tx->tid){ // the current transaction is already having the lock
